...
fetchInsightsBatch()
  .then((allInsights) => {
    ...
    let temp = insightsJSON.map((a, i) => {
      if (a.interactive_component_tap) { return ({ result: a.interactive_component_tap, name: a.campaign_name, age: a.age, gender: a.gender }) }
      else {
        return ({
          result: [{ "value": "0" }], name: a.campaign_name, age: a.age, gender: a.gender
        })
      }
    })
    return (temp)
