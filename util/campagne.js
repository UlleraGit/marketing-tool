...
export async function campaignCreator(name,) {
    ...
    const access_token = 'EAAZA7BZCwWkmUBABPdQGaRxqcRlh9pYzZAhBaOzLkZBD6kpWF2nfDic6KBvmCkjQcYOIpIbMMsh1NGOCLxetacnob98eGeTleVbx6jz4W8UQuZAVmE9KOlZBW1F2uwRS19cjZB7rpm10SZC7HnEIMKJz3LpCRHWSlWNz5o8qDSZBprTPds0wD3qdV';
...
async function adSetCreator(campagne_id) {
    ...
    params = {
        ...
        'targeting': { 'geo_locations': { 'countries': ['AT', 'DE', 'CH'] }, 'age_min': 13, 'age_max': 27, 'publisher_platforms': ['instagram'], 'instagram_positions': ['story'], },
        'status': 'PAUSED',
    };

    const adset = await (new AdAccount(id)).createAdSet(
        fields,
        params
    );
...
async function adCreativeCreator(adset_id) {
    ...
    const adcreative = await (new AdAccount(id)).createAdCreative(
        fields,
        params
    );
